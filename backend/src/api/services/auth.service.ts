...
export class AuthService {
  ...
  async verifyEmail(emailToken: any) {
    try {
      const payload = await this.tokenService.verifyToken(emailToken, "verifyEmail")
      const tokens = await this.tokenService.generateAuthTokens(payload.token)
      return {
        success: true,
        ...tokens
      ...
      throw new ApiError(httpStatus.UNAUTHORIZED, "VerifyEmail token expired, please log in again")
    }
  }
